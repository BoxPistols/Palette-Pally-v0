...
import { useContrastInfo } from "@/hooks/useContrastInfo"
...
interface ColorVariationRowProps {
  name: string
  color: string
  textColor?: string
}

function ColorVariationRow({ name, color, textColor }: ColorVariationRowProps) {
  const { contrast, wcagLevel, levelColor, textColor: finalTextColor } = useContrastInfo(color, textColor)
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1">
        <div style={{ color: finalTextColor }} className="text-xs font-medium">
          {name}: {color}
        </div>
      </div>
      <div className="flex items-center gap-1">
...
export function MUIColorDisplay({ colorData }: MUIColorDisplayProps) {
  if (colorData.type === "simple") {
    ...
    const { contrast, wcagLevel, levelColor, textColor } = useContrastInfo(
      colorData.main,
      ...
      <CardContent className="p-0">
        {variations.map(({ name, color }) => (
          <ColorVariationRow
            ...
            name={name}
            color={color}
            textColor={colorData.contrastText}
          />
        ))}

...
function PaletteApp() {
  ...
  const hasStandardVariations = (color: ColorData): boolean => {
    ...
    if (
      ...
      (color.role === "text" || color.role === "background") // ここで "text" の可能性を考慮している
    ) {
      return false
    }
    // ロールを持つカラーは標準バリエーションを持つと判定（text/backgroundを除く）
    // 型の重複エラー回避のため、color.roleがstring型であることを明示的にチェック
    // かつ、color.roleが"primary" | "secondary" | ... のいずれかであることを型ガードで確認
    // 型の重複エラー回避のため、color.roleがstring型であることを明示的にチェック
    // かつ、color.roleが"primary" | "secondary" | ... のいずれかであることを型ガードで確認
    // さらに、"text" と "background" は ColorRole の型に含まれないため、これらを除外する条件は不要
    return (
      !!color.role &&
  ...
  const _addColor = () => {
    setColorData((prev) => {
      ...
      const newColor = {
        name: `_unused_${newPalette.length + 1}`,
  ...
  // ColorRoleの比較を修正
  const _isSpecialPath = (parentPath: string, tokenName: string): boolean => {
